...
**v0.7.0（鸿蒙反馈第五轮，斜杠那一下）**：真机实测只剩 `/` 触发那一刻的**单次**收键盘（之后继续输入正常）。真机日志 + 官方输入框对照实验（官方输入框打 `/` 不收键盘）把机制收窄到一点：**回写草稿会让应用异步重写隐藏的官方编辑器 DOM**（`EDITOR childList +1`，在我们手势之外发生），ArkWeb 对这种"输入法之外的可编辑面变化"会收键盘；而官方路径里那次变化**就是用户自己的编辑**（同一个手势内），所以没事。
