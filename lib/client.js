window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * `live` — the pre-0.6 behaviour, kept as the A/B control.
    ...
    var TRIGGER_MODES = ["changes", "live", "off"];
    ...
    function createNativeInput(ctx, diary) {
      ...
      var onInput = function (event) {
        ...
        if (!composing && (mode === "live" || (mode === "changes" && triggerChanged))) {
      ...
      var onCompositionEnd = function (event) {
        ...
        if (triggerChanged) chattyMirror(value);
