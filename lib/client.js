window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var INPUT_CSS = [
      ...
      "  font: 11px/1.4 ui-monospace, monospace;",
      "}",
      "/* Device test bench (?dsh-mobile-input=bench): the fields live in ONE fixed",
      "   panel and the running log in ANOTHER, so repainting the log can never",
      "   reflow a field under test (fixed boxes are out of flow). */",
    ...
    function createNativeInput(ctx, diary) {
      ...
      React.useEffect(function () {
        var update = function () { growthRef.current = readGrowthMode(); };
        ...
        return h(
          ...
          h(
            "button",
            {
              ...
              onMouseDown: function (event) { event.preventDefault(); },
              onClick: function () {
                writeDiagnostics(diagnosing ? "off" : "both");
                try {
    ...
    function installBench(diary) {
      ...
      title.textContent = "测试台：每格各输入 3-4 个字，注意键盘会不会自己收起；完成后点「复制日志」把日志发给 AI";
      ...
      diary.watch(log);
      diary.add("bench on; 下面 A-E 每格各输 3-4 字", true);
      return function () {
        diary.unwatch(log);
        panel.remove();
        log.remove();
