  ...
  factory: (require) => {
    ...
    function createNativeInput(ctx) {
      ...
      var measure = function () {
        ...
        var next = [
          scrollRect.left - cardRect.left - card.clientLeft,
          scrollRect.top - cardRect.top - card.clientTop,
          scroll.clientWidth,
          scroll.clientHeight,
        ].join(":");
        if (next !== geometryRef.current) {
      ...
      var onInput = function (event) {
        debug("input " + event.currentTarget.value.length + " chars composing=" + (event.nativeEvent !== undefined && event.nativeEvent.isComposing === true));
